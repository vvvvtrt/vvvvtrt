...
👋 Привет! Меня зовут Михаил. Я — разработчик, специализируюсь на ML/AI-сервисах и backend-разработке.  
...
**ML-инженер в СБЕР** с 01.04.2026 по настоящее время
 - Анализ логов AI-агента: выявление систематических неточностей, проектирование пайплайна метрик и критериев оценки (основа для LLM-as-a-judge и детектора математических ошибок)
 - Исследование импутации пропусков в таблицах: Swing (попарная similarity) → GraphSAGE → MLP с PMM-сэмплированием; дистилляция encoder от top-K соседей Swing через KL-дивергенцию; на инференсе — приближённый поиск через FAISS
 - Pipeline LLM-as-a-judge: разметка аргументаций агента через GigaChat по 9 критериям и дистилляция в ruBERT-tiny2 (10 голов) для прода на CPU через ONNX — полный набор скоров за один forward-пас
 - Каскадный детектор математических ошибок в числовых показателях агента: определение типа формулы, проверка арифметики; точность подклассификаторов до 96.6%
 - Подготовка датасетов для AI-агента на Spark и Hive
...
- 1 место - AI-powered хакатон от Бизнес-инкубатора МГУ
